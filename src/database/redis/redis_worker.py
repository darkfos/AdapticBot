...
class RedisWorker:
    ...
    __redis: Redis = Redis(
        ...
        port=int(DatabaseSettings().redis_port),  # noqa
    )

    @classmethod
    async def set_value(cls, key, value) -> None:
        await cls.__redis.set(
            name=key, value=value, ex=DatabaseSettings().redis_time_life  # noqa
        )
    ...
    async def set_key_tg(cls, key) -> bool:
        ...
        await cls.__redis.set(
            name=key, value=0, ex=DatabaseSettings().redis_time_life
        )  # noqa
    ...
    async def set_message_tg_user(cls, key) -> bool:
        ...
        if user_data > BotGeneralSettings.LIMIT_MESSAGE:
            return False
        await cls.__redis.set(
            name=key, value=user_data + 1, ex=DatabaseSettings().redis_time_life  # noqa
        )
        return True
